...
description: 当本轮对话里出现值得未来读到的开发记忆（稳定决策、用户纠正/偏好、踩过的坑、试错收敛后的最终结论、文档与实测口径漂移的事实校正、新 commit 落盘但 progress 没同步、用户 checkpoint 类发言如"commit 一下/告一段落/明天再继续/记一下"），用本 skill 写进当前仓库+分支的开发记忆。**这是 agent 主动行为，识别到信号即直接调用，不要先问"要不要 capture"** —— 写入是 lazy init、廉价、用户可随时删，不写的代价是下一个接手的 agent（或未来的你）重走同样的弯路。动作前用一句话说明在写什么、走哪个 kind。也覆盖用户显式让记的场景（"记一下"、"留个 note"、"以后都按这个"、"凡是 X 时就 Y"、"立个规矩"）。任何"写入仓库 / 分支记忆"的需求都走这里，dev-memory 套件里**唯一的写入入口**。
---
...
```bash
npx dev-memory capture suggest-kind \
